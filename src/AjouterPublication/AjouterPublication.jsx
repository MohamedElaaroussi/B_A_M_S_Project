...
function AjouterPublication() {
    ...
    const EnvoyerPub = async() => {
      ...
      const IdPublication = 'Pub_' + Math.random()*1000 ;
      const IdContenuPublication = 'ContenuPub_' +  Math.random()*1000 ;
      ...
      if(Element == 'Image'){
        ...
        const LinkImage = "Users/" + Status.OuvrireSession.Id +"/Pubs/" + Status.OuvrireSession.Id +"_Img " + Math.random()*1000 ;
        ...
        uploadBytes(imageRef, image).then(() => {
          getDownloadURL(imageRef)
            .then((url) => {
              console.log(url) ;
              setUrl(url);
              Axios.post("http://localhost:5000/AjouterPublication",{
                Id : Status.OuvrireSession.Id ,
                IdPublication : IdPublication ,
                IdContenuPublication  : IdContenuPublication  ,
                TypePub : "IMAGE",
                url : url ,
        ...
        const LinkVideo = "Users/" + Status.OuvrireSession.Id +"/Pubs/" + Status.OuvrireSession.Id +"_Vid " + Math.random()*1000 ;
   ...
   <div className='EmplacementAjouterPublication'>
   <br className='m-4'/>
   <div className='AddPub m-5 pe-2 pb-3'>

   <div className='cancel-btn1'  ><span onClick={() => {
    Dispatch({type:"visibile"})
   }}><ion-icon name="close-circle-outline"></ion-icon> </span> </div>
